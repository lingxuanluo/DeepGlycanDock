...
import traceback
from utils_glycan.diffusion_utils import modify_conformer, set_time
from utils_glycan.torsion import modify_conformer_torsion_angles
...
def sampling(input_data_list, model, inference_steps, tr_schedule, rot_schedule, tor_schedule, device, t_to_sigma, model_args,
    ...
    for t_idx in range(inference_steps):
        ...
        for complex_graph_batch in loader:
            ...
            with torch.no_grad():
                tr_score, rot_score, tor_score, ligand_pred, atom_close_labels, residue_pred, residue_close_labels = model(complex_graph_batch) 
                ligand_pred_list.append(ligand_pred.detach().cpu())
                residue_pred_list.append(residue_pred.detach().cpu())
    ...
    return data_list, pred_score, ligand_pred_list, residue_pred_list
