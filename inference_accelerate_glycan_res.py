...
from utils_glycan.diffusion_utils import t_to_sigma as t_to_sigma_compl, get_t_schedule
from utils_glycan.sampling import randomize_position, sampling,inferenceFFOptimize
from utils_glycan.utils import get_symmetry_rmsd, remove_all_hs
from score_in_place_dataset.score_dataset import ScreenDataset
from utils_glycan.utils import get_model, ExponentialMovingAverage, analyze_sugar_rings, analyze_pocket_residues
from utils_glycan.visualise import PDBFile
...
parser.add_argument('--keep_input_pose', action='store_true', default=False, help='Whether keep original input pose')
...
parser.add_argument('--cache_path', type=str, default='~/PDBBIND/cache_PDBBIND_pocket_8A2', help='Folder from where to load/restore cached dataset')
...
parser.add_argument('--keep_local_structures', action='store_true', default=False, help='Whether to keep local structures')
...
def main_function():
    ...
    all_molecules = 0
    pbar = tqdm(zip(pocket_paths,ligands_paths,[None],surface_paths,protein_paths,pocket_centers),total=len(pocket_paths))
    ...
    for pocket_path,ligands_path,ref_ligand,surface_path,protein_path,pocket_center in pbar:
        ...
        try:
            ...
            test_dataset = ScreenDataset(pocket_path, ligands_path, ref_ligand, ligand_embeddings, surface_path, pocket_center,transform=None,
                                receptor_radius=confidence_args.receptor_radius,
            ...
            """
            Start sampling conformers by DeepGlycanDock
            """
            for idx, orig_complex_graph in tqdm(enumerate(test_loader),total = len(test_loader),disable= not accelerator.is_local_main_process):
                # logger.warning(orig_complex_graph)
                try:
                        ...
                        try:
                            ...
                            if args.save_visualisation:
                                ...
                                for idx, graph in enumerate(data_list):
                                    # raw pose
                                    # lig = read_mol(args.data_dir, graph['name'].split('//')[-1].split('_ligand_gaff')[0], remove_hs=score_model_args.remove_hs)
                                    lig = read_mol(args.data_dir, f"{graph['name'].split('_')[1]}_{graph['name'].split('_')[2]}", remove_hs=score_model_args.remove_hs)
                                    pdb = PDBFile(lig)
                                    pdb.add(lig, 0, 0)
                                    # pose rdkit matching
                            ...
                            if not args.no_model:
                                ...
                                data_list, confidence, ligand_pred_list, residue_pred_list = sampling(input_data_list=data_list, model=model,
                                ...
                                atom_list = analyze_sugar_rings(data_list, ligand_pred_list)
                            ...
                            if args.save_docking_result:
                                ...
                                for _ in range(len(orig_complex_graph_list)):
                                    for rank, batch_idx in enumerate(re_order[:args.save_docking_result_number]):
                                        ...
                                        atom_info = atom_list[true_idx]
                                        residue_info = residue_list[true_idx]
                                        ...
                                        with open(os.path.join(write_dir, atom_filename.split('/')[-1]), 'w') as f:
                                            f.writelines(atom_info)
                                        with open(os.path.join(write_dir, residue_filename.split('/')[-1]), 'w') as f:
                                            f.writelines(residue_info)

                                        
                                        if args.save_visualisation:
                                            write_dir_vis = f'{args.out_dir}/DeepGlycanDock_docking_result'
                                            os.makedirs(write_dir, exist_ok=True)
                                            logger.warning(f"{write_dir_vis} !!!!")
                                            if args.inference_mode=='evaluate':
                                                vis_filename =f'{data_list[true_idx]["name"].split("//")[-1].split("_ligand_gaff")[0]}_sample_idx_{batch_idx}_rank_{rank + 1}_rmsd_{rmsd}_confidence_{confidence_tmp[batch_idx]}.pdb'
                                            else:
                                                vis_filename = f'{data_list[true_idx]["name"].split("//")[-1].split("_ligand_gaff")[0]}_sample_idx_{batch_idx}_rank_{rank + 1}_confidence_{confidence_tmp[batch_idx]}.pdb'
                                            try:
                                                visualization_list[batch_idx].write(
                                                    f'{write_dir_vis}/{vis_filename}')
                                            except:
                                                logger.warning(f"{str(visualization_list)} !!!!")
                                                traceback.print_exc()
                                                continue
                                    head_threshold += N
                                    tail_threshold += N
