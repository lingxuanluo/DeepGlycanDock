...
def compute_inp_surface(target_filename, ligand_filename, out_dir = None, dist_threshold=10):
    ...
    if out_dir is not None: 
        ...
        sufix = '/' + os.path.splitext(target_filename)[0].split('/')[-1] + '_'+os.path.splitext(ligand_filename)[0].split('/')[-1] + '_'+str(dist_threshold)+'A.pdb'
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ligand_file', type=str, required=True, help='Path to the input ligand file (SDF or MOL2)')
    parser.add_argument('--out_dir', type=str, default='./surface_output', help='Output directory for surface files')
    args = parser.parse_args()
    # make args.pdb_file and args.ligand_file in absolute path
    
    args.pdb_file = os.path.abspath(args.pdb_file)
    args.ligand_file = os.path.abspath(args.ligand_file)
    ...
    if not os.path.exists(args.ligand_file):
        raise FileNotFoundError(f"Ligand file {args.ligand_file} does not exist")

    # Process the single input
    try:
        res = compute_inp_surface(args.pdb_file, args.ligand_file, args.out_dir, dist_threshold=8)
        print(f"Processed: {args.pdb_file}, {args.ligand_file}")
    except Exception as e:
        print(f"Error processing {args.pdb_file}, {args.ligand_file}: {str(e)}")
