...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
                 ...
                 atom_radius=5, atom_max_neighbors=None, esm_embeddings_path=None, require_ligand=False, ligand_embeddings_path=None,
        ...
        self.full_cache_path = os.path.join(cache_path, f'limit{self.limit_complexes}'
                                            ...
                                            + ('' if self.ligand_embeddings_path is None else f'_ligandEmbeddings')
    ...
    def get_complexs_list(self, num):
        ...
        for idx in range(min(num, len(glob.glob(os.path.join(self.full_cache_path,'heterographs*.pkl'))))):
            # print(idx, idx, idx, idx, 'idx')
    ...
    def preprocessing(self):
        ...
        if self.esm_embeddings_path is not None:
            ...
            complex_names_all = [name for name in embedding_names if name.split('_protein')[0] in set(complex_names_all)]
            # logger.info(f'complex_names_all: {str(complex_names_all)}')
            complex_names_all_new = []
            for name in complex_names_all:
                try:
                    lm_embeddings_chains_all.append(id_to_embeddings[name])
                    complex_names_all_new.append(name.split('_protein')[0])
                except:
                    continue
            ...
            logger.info(f'Final loading {len(complex_names_all)} complexes.')
        else:
            lm_embeddings_chains_all = [None] * len(complex_names_all)
        if self.ligand_embeddings_path is not None:
            logger.info(f'Loading ligand embeddings from {self.ligand_embeddings_path}')
            ligand_embeddings = pickle.load(open(self.ligand_embeddings_path, 'rb'))
            ligand_embedding_names = list(ligand_embeddings.keys())
            ligand_name_new = []
            ligand_embeddings_all = []
            ligand_names_all = [name for name in ligand_embedding_names if name.split('_ligand')[0] in set(complex_names_all)]
            for name in ligand_names_all:
                try:
                    ligand_name_new.append(name.split('_ligand')[0])
                    ligand_embeddings_all.append(ligand_embeddings[name])
                except:
                    continue
            assert len(ligand_name_new) == len(complex_names_all), f'len(ligand_name_new){len(ligand_name_new)} != len(complex_names_all){len(complex_names_all)}'
            logger.info(f'Final loading {len(ligand_name_new)} ligand embeddings.')
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(complex_names_all)//1000+1):
                ...
                ligand_embeddings = ligand_embeddings_all[1000*i:1000*(i+1)]
                complex_graphs, rdkit_ligands = [], []

                with tqdm(total=len(complex_names), desc=f'loading complexes {i}/{len(complex_names_all)//1000+1}') as pbar:
                    # map_fn = p.imap_unordered if self.num_workers > 1 else map
                    t_list = Parallel(n_jobs=self.num_workers, backend="multiprocessing")(delayed(self.get_complex)(x) for x in tqdm(zip(complex_names, lm_embeddings_chains, ligand_embeddings, [None] * len(complex_names), [None] * len(complex_names)), total=len(complex_names)))
                    for t in t_list:
                        complex_graphs.extend(t[0])
                        rdkit_ligands.extend(t[1])
                        pbar.update()
                dump_path = os.path.join(self.full_cache_path, f"heterographs{i}.pkl")
                logger.info(f'dump complexes to {dump_path}, {str(complex_graphs)}')
            ...
            with tqdm(total=len(complex_names_all), desc='loading complexes') as pbar:
                for t in map(self.get_complex, zip(complex_names_all, lm_embeddings_chains_all, ligand_embeddings_all, [None] * len(complex_names_all), [None] * len(complex_names_all))):
        ...
        for idx, ligand_description in tqdm(enumerate(self.ligand_descriptions)):
            try:
                ...
                if mol is not None:
                    ...
                    unimol_repr = clf.get_repr(ligand_description)
                    cls_repr = np.array(unimol_repr["cls_repr"])
                    ligand_embeddings_list.append(cls_repr)
                else:
        ...
        if self.num_workers > 1:
            ...
            for i in range(len(self.protein_path_list)//1000+1):
                ...
                if self.num_workers > 1:
                    ...
                    p.__enter__()
                with tqdm(total=len(protein_paths_chunk), desc=f'loading complexes {i}/{len(protein_paths_chunk)//10+1}') as pbar:
                    map_fn = p.imap_unordered if self.num_workers > 1 else map
                    for t in map_fn(self.get_complex, zip(protein_paths_chunk, lm_embeddings_chains, ligand_embeddings_list, ligands_chunk,ligand_description_chunk)):
                        complex_graphs.extend(t[0])
                        rdkit_ligands.extend(t[1])
                        pbar.update()
            ...
            with tqdm(total=len(self.protein_path_list), desc='loading complexes') as pbar:
                for t in map(self.get_complex, zip(self.protein_path_list, lm_embeddings_chains_all, ligand_embeddings_list, ligands_list, self.ligand_descriptions)):
                    complex_graphs.extend(t[0])
                    rdkit_ligands.extend(t[1])
                    pbar.update()
    ...
    def get_complex(self, par):
        name, lm_embedding_chains, ligand_embeddings_list, ligand, ligand_description = par
        ...
        if ligand is not None:
            ...
            try:
                # logger.info(f'{self.surface_path}/{name}/*.pdb')
        ...
        for i, lig in enumerate(ligs):
            ...
            try:
                get_lig_graph_with_matching(lig, complex_graph, ligand_embeddings_list[i], self.popsize, self.maxiter, self.matching, self.keep_original,
...
def construct_loader(args, t_to_sigma):
    ...
    common_args = {'transform': transform, 'root': args.data_dir, 'limit_complexes': args.limit_complexes,
                   ...
                   'esm_embeddings_path': args.esm_embeddings_path,'surface_path':args.surface_path, 'ligand_embeddings_path':args.ligand_embeddings_path}
    ...
    # prefetch_factor = 0
    train_loader = loader_class(dataset=train_dataset, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=True, pin_memory=args.pin_memory, prefetch_factor = 2, drop_last = False)
